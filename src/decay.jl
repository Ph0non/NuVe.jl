...
function getSampleFromSource(nv::Symbol)
	SQLite.query(nvdb, "select s_id, " * array2string(nu_names) *
...
aus den Einstellungen bezogen ([`Settings`](@ref)).
"""
...
"""
function decay_correction()
	sample = getSampleFromSource(setting.nv)
	sample_array = convert(Array, sample[map(x -> Symbol(x), nu_names)])
	diff_days = map(x -> x.value, diffDays(getSampleDate(setting.nv)))
	hl_array = convert(Array{Float64}, hl)

	sample_decay = Dict()
	for (index, i) in enumerate(getInterval())
		push!(sample_decay, string(i) => DataFrame([sample.s_id sample_array .* 2 .^ (-diff_days[:,index] ./ hl_array)], names(sample) ))
